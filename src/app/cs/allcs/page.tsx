...
import { getAllcs } from "@/services/cs";
//store
...
function AllcsPage() {
  ...
  const changeSubmit = async (id: string, data: string) => {
    ...
    await getAllcs(currentPage, status, tag)
  }
  ...
  const GetAllpurchase = useCallback(async () => {
    const purchase = await getAllcs(currentPage, status, tag);
