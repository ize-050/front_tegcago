...
import { useForm, Controller, SubmitHandler, FormProvider, useFieldArray, useWatch } from "react-hook-form"
...
const Purchase = () => {
    ...
    return (
        <>
            <div className="p-5 mx-auto  text-black">
                ...
                <div className=" flex  flex-col  md:flex-row  mt-5">
                    <div className="w-full md:w-1/3 flex flex-col">
                        <label className="block mb-2  text-gray-500  text-sm font-semibold">เลขตีราคา</label>
                        <p>{data?.book_number}</p>
                    </div>
                    <div className="w-full md:w-1/3 flex flex-col">
                        <label className="block mb-2  text-gray-500  text-sm font-semibold">ชื่อเซลล์</label>
                    ...
                    <div className="w-full md:w-1/3 flex flex-col">
                        <label className="block mb-2  text-gray-500  text-sm font-semibold">วันที่/เวลา</label>
                ...
                <div className=" flex  flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-4 mt-5">
                    ...
                    <div className="w-full md:w-1/2  flex flex-col">
                        <label
                            className="block mb-2  text-gray-500  text-sm font-semibold">บริการหัวรถลาก</label>
                        <p>{data?.d_truck}</p>
                    </div>
                </div>
                ...
                <div className=" flex  flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-4 mt-5">
                    ...
                    <div className="w-full md:w-1/2 flex flex-col">
                        <label
                            className="block mb-2  text-gray-500  text-sm font-semibold">ที่อยู่ปลายทาง </label>
                ...
                <div className=" flex  flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-4 mt-5 ">
                    <div className="w-full md:w-1/2 flex flex-col">
                        <label className="block mb-2  text-gray-500  text-sm font-semibold">Refund Tax
                            ต้นทาง</label>
                ...
                <div className="flex flex-row ">
                    <h2 className="flex-1 w-1/2 mb-5  justify-start text-2xl">ข้อมูลของ Agency</h2>
                    ...
                    <div className="flex-1  w-1/2 justify-end text-right right-0">
                        {!Agentdetail ? (
                            ...
                            <>
                                <Button className="bg-[#FFFFFF] text-[#305D79] border-2  border-[#305D79]  font-bold uppercase text-sm px-6 py-2 rounded  outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150 "
                                 type="button"
                                    onClick={() => {
                                        changeAgentCy(false)
                                    }}
                            ...
                            <div className="bg-gray-200 p-5 border rounded-md shadow-sm mb-5">
                                <div className="flex justify-between items-center mb-4">
                                    <h2 className="text-black  font-semibold  text-lg">
                                        <p className="text-red-500 ">(*Sales เลือก) </p>   {data?.d_sale_agentcy[0]?.d_agentcy?.agentcy?.agent_name}
                                    </h2>
                                    <button type="button"
                                        onClick={() => {
                                            dispatch(setModalViewAgentCy(true))
                                            dispatch(setAgentCyDetail(data?.d_sale_agentcy[0]?.d_agentcy))
                                        }}
                                        className="text-blue-500 items-end underline">รายละเอียด</button>
                                </div>
                                ...
                                <div className="flex w-full">
                                    ...
                                    <div className="flex  w-2/4">
                                        <div className="flex w-1/4 flex-col  items-center mb-2">
                                            <p className="text-gray-600 ">สายเรือ: {data?.d_sale_agentcy[0]?.d_agentcy.agent_boat}</p>
                                            <p className="text-gray-600 ">TIT: {data?.d_sale_agentcy[0]?.d_agentcy.agentcy_tit}</p>
                                        </div>
                                        <div className="flex  w-1/4 flex-col  items-center mb-2">
                                            <p className="text-gray-600w-28 ">ETD:{data?.d_sale_agentcy[0]?.d_agentcy.agentcy_etd}</p>
                                            <p className="text-gray-600 truncate">ETA: {data?.d_sale_agentcy[0]?.d_agentcy.agentcy_eta}</p>
                                        </div>
                                        {/*<p className="text-gray-600">หมายเหตุ: {res.agentcy_etc}</p>*/}
                                    </div>
                                ...
                                <div className="flex flex-col items-end">
                                    <p className="text-gray-600">ยอดค่าใช้จ่ายจริง</p>
                                    <p className="text-xl font-bold">{data?.d_sale_agentcy[0]?.d_agentcy?.d_agentcy_detail?.reduce(
                                        (accumulator: number, item: any) => accumulator + Number(item.d_net_balance),
                                        0 // Initial value for the accumulator
                                    )} ฿</p>
                                </div>
            ...
            {modalAgentcy &&
                <ModalAddAgentcy></ModalAddAgentcy>
            }
            <PaymentComponent id={data.id} payment={data.payment_purchase}></PaymentComponent>
            <br></br>
