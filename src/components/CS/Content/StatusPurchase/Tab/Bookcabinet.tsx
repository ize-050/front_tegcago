...
import { statusOrderData, setEditForm, setStoreTabActive } from "@/stores/statusOrder"
...
const BookcabinetComponent = ({ purchase }: {
    purchase: any
}) => {
                ...
                <div className="mx-auto text-black">
                    <div className="flex bg-gray-50">
                        <div className=" flex-1 w-50  px-5  rounded-md">
                            <h1 className="mb-5  text-1xl font-semibold">รายละเอียดการจองตู้</h1>
                        </div>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="flex">
                            <div className="w-1/2 p-5">
                                <label className="block mb-2 text-lg text-gray-500  sm:text-sm font-semibold">วันที่รับเรื่อง</label>
                                        ...
                                        <Controller
                                            ...
                                            render={({ field: { onChange, onBlur, value } }) => (
                                                <input
                                                    ...
                                                    onChange={onChange}
                                                    type="date" className={`
                                            ${errors.date_receiving ? 'border-red-500' : 'border-gray-200'}
                                            px-4 py-2 outline-none rounded-md w-full`} />
                                            )}
                                        />
                                        {errors.date_receiving && <p className="text-red-500">กรุณากรอกข้อมูล.</p>}
                                    </>
                                    : <p>{data?.date_receiving}</p>}
                            </div>
                            <div className="w-1/2 p-5">
                                <label className="block mb-2 text-lg text-gray-500  sm:text-sm font-semibold">วันที่ Booking</label>
                        ...
                        <div className="flex">
                            ...
                            <div className="w-1/2 p-5">
                                <label className="block mb-2 text-lg text-gray-500  sm:text-sm font-semibold">Agency</label>
                                <p className="text-black">{PurchaseData?.d_sale_agentcy[0]?.d_agentcy?.agentcy.agent_name}</p>
                            </div>
                            <div className="w-1/2 p-5">
                                <label className="block mb-2 text-lg text-gray-500  sm:text-sm font-semibold">สายเรือ</label>
                                <p className="text-black">{PurchaseData?.d_sale_agentcy[0]?.d_agentcy?.agent_boat}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex">
                            ...
                            <div className="w-1/2 p-5">
                                <label className="block mb-2 text-lg text-gray-500  sm:text-sm font-semibold">วันที่เข้าบรรจุตู้</label>
                                        ...
                                        <Controller
                                            ...
                                            render={({ field: { onChange, onBlur, value } }) => (
                                                <input
                                                    ...
                                                    onChange={onChange}
                                                    type="date" className={`
                                            ${errors.date_entering ? 'border-red-500' : 'border-gray-200'}
                                            px-4 py-2 outline-none rounded-md w-full`} />
                                            )}
                                        />
                                        {errors.date_entering && <p className="text-red-500">กรุณากรอกข้อมูล.</p>}
                                    </>
                        ...
                        <div className="">
                            <div className="p-5">
                                <label className="block mb-2 text-lg text-gray-500  sm:text-sm font-semibold">รูปภาพ</label>
                                {dataStatus.type !== "view" ?
                                    <>
                                        <div className="w-1/2">
                                    ...
                                    :
                                    <>
                                        {data?.Bookcabinet_picture?.map(async (images: any, index: number) => {

                                            const isExcel = images.picture_name?.endsWith('.xlsx') || images.picture_name?.endsWith('.xls') || images.picture_name?.endsWith('.csv');
                                            const isPdf = images.picture_name?.endsWith('.pdf');
                                            const isImage = images.picture_name?.endsWith('.jpg') || images.picture_name?.endsWith('.png');
                                            const url = process.env.NEXT_PUBLIC_URL_API + "/" + images.picture_path
                                                    ...
                                                    {isExcel && (
                                                        <div>
                                                            <div className="relative w-full h-32  border-2   overflow-hidden">
                                                                ...
                                                                <h3 className="text-sm font-semibold mb-2">{images.picture_name}</h3>
                                                                <div
                                                                    className="absolute inset-0 bg-black bg-opacity-50 opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
