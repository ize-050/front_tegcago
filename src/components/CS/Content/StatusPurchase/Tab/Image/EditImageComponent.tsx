...
const EdituploadComponent = ({
  ...
  const filteredImages = image?.filter((img: any) => img.key === name) || [];
  ...
  useEffect(() => {
    if (image) {
      let newFiles = filteredImages
        .filter((res) => res.key === name)
        .map((file: any) => {
          if (file instanceof File) {
            ...
            return {
              ...
              url: `${
                file.file_path
                  ? process.env.NEXT_PUBLIC_URL_API + file.file_path
                  : process.env.NEXT_PUBLIC_URL_API + file.picture_path
              }`,
              type: "image/png",
