...
import { createRecordMoney, updateRecordMoney, getRecordMoneyById, RecordMoneyData, CustomerDepositData, ExchangeData } from '@/services/record-money';

interface CustomerDepositFormData {
...
interface RecordMoneyForm {
    ...
    customerDeposit?: CustomerDepositFormData;
...
const ModalRecordMoneyComponent: React.FC = () => {
            ...
            if (editRecord?.id) {
                try {
                    ...
                    if (response && response.data) {
                        ...
                        // ดึงข้อมูลหัวข้อการฝากจาก customerDeposit ถ้ามี
                        if (recordData.customerDeposit?.deposit_purpose) {
                            console.log('Setting deposit_purpose from customerDeposit:', recordData.customerDeposit.deposit_purpose);
                            setValue('deposit_purpose', recordData.customerDeposit.deposit_purpose);
                        } else {
                        ...
                        if (recordData.customerDeposit) {
                            ...
                            // แปลงวันที่โอนให้อยู่ในรูปแบบ YYYY-MM-DD สำหรับ input type="date"
                            if (recordData.customerDeposit.transferDate) {
                                const formattedTransferDate = new Date(recordData.customerDeposit.transferDate).toISOString().split('T')[0];
                                console.log('Setting customer deposit transfer date to:', formattedTransferDate);
                                setValue('customerDeposit.transferDate', formattedTransferDate);
                            } else {
                                setValue('customerDeposit.transferDate', new Date().toISOString().split('T')[0]);
                            }
                        ...
                        if (recordData.exchange) {
                            ...
                            // แปลงวันที่โอนให้อยู่ในรูปแบบ YYYY-MM-DD สำหรับ input type="date"
                            if (recordData.exchange.transferDate) {
                                const formattedTransferDate = new Date(recordData.exchange.transferDate).toISOString().split('T')[0];
                                console.log('Setting exchange transfer date to:', formattedTransferDate);
                                setValue('exchange.transferDate', formattedTransferDate);
                            } else {
                                setValue('exchange.transferDate', new Date().toISOString().split('T')[0]);
                            }
                        ...
                        // ดึงข้อมูลรูปภาพจากทุกแหล่งที่เป็นไปได้
                        let transferSlipUrl = null;
                        let transferSlipUrlExchange = null;
                        
                        // ตรวจสอบจาก recordData.transferSlipUrl
                        if (recordData.transferSlipUrl) {
                            console.log('Found transferSlipUrl in recordData:', recordData.transferSlipUrl);
                            transferSlipUrl = process.env.NEXT_PUBLIC_URL_API + recordData.transferSlipUrl;
                        }
                        // ตรวจสอบจาก customerDeposit.transferSlipUrl
                        if (recordData.customerDeposit?.transferSlipUrl) {
                            console.log('Found transferSlipUrl in customerDeposit:', recordData.customerDeposit.transferSlipUrl);
                            transferSlipUrl = process.env.NEXT_PUBLIC_URL_API + recordData.customerDeposit.transferSlipUrl;
                        }
                        // ตรวจสอบจาก exchange.transferSlipUr
                      
                         if (recordData.exchange?.transferSlipUrl) {
                            console.log('Found transferSlipUrl in exchange:', recordData.exchange.transferSlipUrl);
                            transferSlipUrlExchange = process.env.NEXT_PUBLIC_URL_API + recordData.exchange.transferSlipUrl;
                        }
                     
                        
                            // ตั้งค่าตามประเภทรายการ
                            
                                setValue('customerDeposit.existingTransferSlip', transferSlipUrl);

                                setValue('exchange.existingTransferSlip', transferSlipUrlExchange);
    ...
    const onSubmit = async (data: RecordMoneyForm) => {
        try {
            setLoading(true);
            ...
            if (data.deposit_purpose) {
                formData.append('deposit_purpose', data.deposit_purpose);
            }
            ...
            // เพิ่มไฟล์หลัก (ถ้ามี)
            const uploadFile = document.querySelector('input[name="file"]') as HTMLInputElement;
            if (uploadFile && uploadFile.files && uploadFile.files.length > 0) {
                formData.append('file', uploadFile.files[0]);
            }
            
            // เพิ่มข้อมูล customerDeposit
            if (data.customerDeposit) {
                // เพิ่มข้อมูล customerDeposit ทีละฟิลด์
                if (data.customerDeposit.amountRMB) formData.append('customerDeposit.amountRMB', data.customerDeposit.amountRMB.toString());
                if (data.customerDeposit.priceDifference) formData.append('customerDeposit.priceDifference', data.customerDeposit.priceDifference.toString());
                if (data.customerDeposit.exchangeRate) formData.append('customerDeposit.exchangeRate', data.customerDeposit.exchangeRate.toString());
                if (data.customerDeposit.fee) formData.append('customerDeposit.fee', data.customerDeposit.fee.toString());
                if (data.customerDeposit.amount) formData.append('customerDeposit.amount', data.customerDeposit.amount.toString());
                if (data.customerDeposit.vat) formData.append('customerDeposit.vat', data.customerDeposit.vat.toString());
                if (data.customerDeposit.totalWithVat) formData.append('customerDeposit.totalWithVat', data.customerDeposit.totalWithVat.toString());
                
                if (data.customerDeposit.transferDate) {
                    formData.append('customerDeposit.transferDate', data.customerDeposit.transferDate);
                }
                
                if (data.customerDeposit.receivingAccount) formData.append('customerDeposit.receivingAccount', data.customerDeposit.receivingAccount.toString());
                if (data.customerDeposit.exchangeRateProfit) formData.append('customerDeposit.exchangeRateProfit', data.customerDeposit.exchangeRateProfit.toString());
                if (data.customerDeposit.incomePerTransaction) formData.append('customerDeposit.incomePerTransaction', data.customerDeposit.incomePerTransaction.toString());
                if (data.customerDeposit.notes) formData.append('customerDeposit.notes', data.customerDeposit.notes.toString());
                if (data.customerDeposit.totalDepositAmount) formData.append('customerDeposit.totalDepositAmount', data.customerDeposit.totalDepositAmount.toString());
                if (data.customerDeposit.includeVat !== undefined) formData.append('customerDeposit.includeVat', data.customerDeposit.includeVat.toString());
                
                // เพิ่มไฟล์ customerDeposit (ถ้ามี)
                if (data.customerDeposit.files && data.customerDeposit.files.length > 0) {
                    formData.append('customerDepositFile', data.customerDeposit.files[0]);
                }
                
                // เพิ่มข้อมูล transferSlipUrl เดิม (ถ้ามี)
                if (data.customerDeposit.existingTransferSlip) {
                    formData.append('existingCustomerDepositTransferSlip', data.customerDeposit.existingTransferSlip);
                }
            }
            
            // เพิ่มข้อมูล exchange
            if (data.exchange) {
                // เพิ่มข้อมูล exchange ทีละฟิลด์
                if (data.exchange.amountRMB) formData.append('exchange.amountRMB', data.exchange.amountRMB.toString());
                if (data.exchange.priceDifference) formData.append('exchange.priceDifference', data.exchange.priceDifference.toString());
                if (data.exchange.exchangeRate) formData.append('exchange.exchangeRate', data.exchange.exchangeRate.toString());
                if (data.exchange.fee) formData.append('exchange.fee', data.exchange.fee.toString());
                if (data.exchange.amount) formData.append('exchange.amount', data.exchange.amount.toString());
                if (data.exchange.vat) formData.append('exchange.vat', data.exchange.vat.toString());
                if (data.exchange.totalWithVat) formData.append('exchange.totalWithVat', data.exchange.totalWithVat.toString());
                
                if (data.exchange.transferDate) {
                    formData.append('exchange.transferDate', data.exchange.transferDate);
                }
                
                if (data.exchange.receivingAccount) formData.append('exchange.receivingAccount', data.exchange.receivingAccount.toString());
                if (data.exchange.exchangeRateProfit) formData.append('exchange.exchangeRateProfit', data.exchange.exchangeRateProfit.toString());
                if (data.exchange.incomePerTransaction) formData.append('exchange.incomePerTransaction', data.exchange.incomePerTransaction.toString());
                if (data.exchange.notes) formData.append('exchange.notes', data.exchange.notes.toString());
                
                // เพิ่มไฟล์ exchange (ถ้ามี)
                if (data.exchange.files && data.exchange.files.length > 0) {
                    formData.append('exchangeFile', data.exchange.files[0]);
                }
                
                // เพิ่มข้อมูล transferSlipUrl เดิม (ถ้ามี)
                if (data.exchange.existingTransferSlip) {
                    formData.append('existingExchangeTransferSlip', data.exchange.existingTransferSlip);
                }
            }
            
            // เพิ่ม ID สำหรับการอัปเดต (ถ้ามี)
            if (editRecord?.id) {
                formData.append('id', editRecord.id.toString());
            }
            
            // ตรวจสอบว่ามี transferSlipUrl เดิมหรือไม่
            if (editRecord?.transferSlipUrl) {
                formData.append('existingTransferSlip', editRecord.transferSlipUrl);
            }
            ...
            if (editRecord?.id) {
                ...
                response = await updateRecordMoney(editRecord.id, formData);
            } else {
                // สร้างข้อมูลใหม่
                response = await createRecordMoney(formData);
            }
                                            ...
                                            <div className="grid grid-cols-1 gap-6 mt-4">
                                                <div>
                                                    ...
                                                    <UploadImageComponent
                                                        ...
                                                        existingImage={watch('customerDeposit.existingTransferSlip') || (editRecord?.id && editRecord?.type === 'deposit' ? watch('exchange.existingTransferSlip') : undefined)}
                                                    />
                                                </div>
                                            </div>
