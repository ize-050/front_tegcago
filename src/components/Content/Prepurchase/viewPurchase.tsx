...
const ViewPrePurchase = ({ purchase }: any) => {
  ...
  return (
    <>
      ...
      <div className=" flex  flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-4 mt-5">
      <div className="w-full md:w-1/3 flex flex-col">
          <label className="block mb-2 text-lg text-gray-500  text-sm font-semibold">กลุ่มประเภทงาน</label>
      ...
      <div className="flex  flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-4 mt-5">
        ...
        <div className="w-full md:w-1/2 flex flex-col">
          ...
          <div className="flex flex-row">
            ...
            {data?.d_product?.d_product_image.map(async (images: any, index: number) => {

              const isExcel = images.name?.endsWith('.xlsx') || images.name?.endsWith('.xls') || images.name?.endsWith('.csv');
              const isPdf = images.name?.endsWith('.pdf');
              const isImage = images.name?.endsWith('.jpg') || images.name?.endsWith('.png');
                  ...
                  {isExcel && (
                    <div>
                      <div className="relative w-full h-32  border-2   overflow-hidden">
                        ...
                        <h3 className="text-sm font-semibold mb-2">{images.name}</h3>
                        <div
                          className="absolute inset-0 bg-black bg-opacity-50 opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
