...
const CSDashboard: React.FC = () => {
  ...
  const [kpiData, setKpiData] = useState<CSKPIData | null>(null);
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${process.env.NEXT_PUBLIC_URL_API}/dashboard/cs/kpis`, {
          headers: {
        ...
        if (response.data.success && response.data.data) {
          ...
          throw new Error(response.data.message || 'Failed to load CS dashboard data');
        }
      } catch (error) {
        console.error('Error loading CS dashboard data:', error);
  ...
  const kpiCards: CSKPICard[] = [
    {
      id: '1',
      title: 'งานรวมทั้งหมด',
      value: kpiData ? kpiData.totalTickets.toLocaleString() : '0',
      subtitle: 'งานทั้งหมดในระบบ',
    ...
    },
    {
      ...
      value: kpiData ? kpiData.resolvedTickets.toLocaleString() : '0',
      subtitle: 'งานที่ดำเนินการเสร็จ',
    ...
    },
    {
      ...
      value: kpiData ? kpiData.pendingTickets.toLocaleString() : '0',
      subtitle: 'งานที่รอการประมวลผล',
    ...
    },
    {
      ...
      value: kpiData ? `${kpiData.customerSatisfaction}%` : '0%',
      subtitle: 'คะแนนความพึงพอใจลูกค้า',
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">CS Dashboard</h2>
            <p className="text-sm text-gray-500">ภาพรวมการดำเนินงาน Customer Service</p>
          </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">ผลงาน Customer Service</h3>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ความพึงพอใจ</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">เวลาแก้ไข</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              <tr className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">งานประจำ</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">2 ชั่วโมง</td>
              </tr>
            </tbody>
          </table>
        </div>
